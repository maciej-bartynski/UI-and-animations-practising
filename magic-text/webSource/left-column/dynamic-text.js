'use strict';
document.addEventListener('DOMContentLoaded', initializeWizardBtn); 
function initializeWizardBtn(){
    let currentSection = document.querySelector('#bodyOfMagicTxt');
    let wizardBtn=document.querySelector('#cv-magictxt #wizardBtn');
    currentSection.addEventListener('mouseover', alertForBtn);
    function alertForBtn(){
        wizardBtn.classList.add('isActive');
    };
    currentSection.addEventListener('mouseout', function(){
        wizardBtn.classList.remove('isActive');
    })
    wizardBtn.addEventListener('click', function(){
        wizardBtn.classList.remove('isActive');
        initializeMagicText();
        currentSection.removeEventListener('mouseover', alertForBtn);
    })
}
let iterator = 0;
function initializeMagicText() {
    let theSpans = document.querySelectorAll('.spanLine');
    for (let i =0; i<theSpans.length; i++){
        let theSpan=theSpans[i];
        if (theSpan.children.length>1){
            let insiders = theSpan.querySelectorAll('SPAN');
            for (let j=0;j<insiders.length;j++){
                let insider = insiders[j];
                theSpan.removeChild(insider);
            }
        }
    }
...
function setMagicText() {
    let leftColumn = document.querySelector("#cv-magictxt .section_left-side");
    ...
    let strings = [
        ...
        ['learningProgress', '(); ', ' // still learning'],
    ];
    let lineIterator = 0;
    let wordIterator = 0;
